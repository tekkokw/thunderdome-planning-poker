...
func (s *Service) CheckinCreate(ctx context.Context, teamID string, userID string, eventValue string) (any, []byte, error, bool) {
	...
	err = s.CheckinService.CheckinCreate(context.Background(), teamID, c.UserID, c.CheckinDate, c.Yesterday, c.Today, c.Blockers, c.Discuss, c.GoalsMet, c.LinearCycleID, c.PostedByUserID)
	if err != nil {
		return nil, nil, err, false
	}
